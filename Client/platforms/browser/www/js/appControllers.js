...
app.controller("Loader", ["$rootScope", "$scope", "$location", "$mdSidenav", "BoxtService", "$mdDialog", "$timeout", LoaderController]);
app.controller("Lobby", ["$rootScope", "$scope", "$location", "$mdSidenav", "BoxtService", "$mdDialog", "$timeout", LobbyController]);
app.controller("GameItem", ["$rootScope", "$scope", "$location", "$mdSidenav", "BoxtService", "$routeParams", "$timeout", GameItemController]);
...
function LoaderController($rootScope, $scope, $location, $mdSidenav, $BoxtService, $mdDialog, $timeout) {
...
function LobbyController($rootScope, $scope, $location, $mdSidenav, $BoxtService, $mdDialog, $timeout) {
    ...
    $scope.gridItemClick = function (item) {
        console.log(item);
        $BoxtService.navigate("game/" + item.GameType);
    };
    $BoxtService.listUserGames(null, function (success) {
    ...
    $BoxtService.listGameInvites(function (success) {
        success.forEach(function (item) {
            $scope.gameInvites.push(item);
        });
    }, function (error) {
        console.error(error);
    });
    ...
    $scope.acceptInvite = function (invite) {
        $BoxtService.acceptInvite(invite.Id, function (success) {
            console.log(success);
        }, function (error) {
            console.log(error);
        });
...
function GameItemController($rootScope, $scope, $location, $mdSidenav, $BoxtService, $routeParams, $timeout) {
    var param = $routeParams.gameType;
    $scope.game = $BoxtService.getGame(param);
